...
def matrix_eigenvalues(p, M):
    ...
    for i in range(M - 6):
        matrix[i, i + 6] = D(alpha, i + 3)
        matrix[i + 6, i] = D(alpha, i + 3)

    # top-right
    matrix[0][M - 3] = C(alpha, M-1) * np.exp(-1j * eta)
    matrix[1][M - 2] = C(alpha, M) * np.exp(-1j * eta)
    matrix[2][M - 1] = C(alpha, 1) * np.exp(-1j * eta)

    matrix[0][M - 6] = D(alpha, M-3)
    matrix[1][M - 5] = D(alpha, M-2)
    matrix[2][M - 4] = D(alpha, M-1)
    matrix[3][M - 3] = D(alpha, M)
    matrix[4][M - 2] = D(alpha, 1)
    matrix[5][M - 1] = D(alpha, 2)

    # bottom-left
    matrix[M - 3][0] = C(alpha, M-1) * np.exp(1j * eta)
    matrix[M - 2][1] = C(alpha, M) * np.exp(1j * eta)
    matrix[M - 1][2] = C(alpha, 1) * np.exp(1j * eta)

    matrix[M - 6][0] = D(alpha, M - 3) * np.exp(1j * eta)
    matrix[M - 5][1] = D(alpha, M - 2) * np.exp(1j * eta)
    matrix[M - 4][2] = D(alpha, M - 1) * np.exp(1j * eta)
    matrix[M - 3][3] = D(alpha, M) * np.exp(1j * eta)
    matrix[M - 2][4] = D(alpha, 1)
    matrix[M - 1][5] = D(alpha, 2)
