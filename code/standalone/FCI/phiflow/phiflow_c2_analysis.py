...
if __name__ == '__main__':
    ...
    phiflow_file = f'charge_pump_FerHofSqu1_chi_100_t1_1_V_10_Coulomb_1_n_2_105_nphi_8_15_LxMUC_1_Ly_14_phi_0_7_71.dat'
    phiflow_path = os.path.join(phiflow_dir, phiflow_file)
    with open(phiflow_path, 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter='\t')
        phi = []
        ...
        for row in plots:
            ...
            charge.append(float(row[1]))
    charge = [i - charge[0] for i in charge]
    ax2.plot(phi, charge, '.', c=f'C2', marker=markers[9], fillstyle='none', markersize=5, markeredgewidth=0.2)
    ...
    with open(phiflow_path, 'r') as csvfile:
        ...
        for row in plots:
            ...
            charge.append(float(row[1]))
    charge = [i - charge[0] for i in charge]
    ax2.plot(phi, charge, '.', c=f'C3', marker=markers[9], fillstyle='none', markersize=5, markeredgewidth=0.2)
    ...
    with open(phiflow_path, 'r') as csvfile:
        ...
        for row in plots:
            ...
            charge.append(float(row[1]))
    charge = [i - charge[0] for i in charge]
    ax3.plot(phi, charge, '.', c=f'C1', marker=markers[1], fillstyle='none', markersize=5, markeredgewidth=0.2)
    ...
    nphi_legend_elements = [Line2D([0], [0], linestyle='none', marker=markers[0], color='k', label='$3/5$', fillstyle='none', markersize=5),
    ...
    leg1 = ax3.legend(handles=nphi_legend_elements, loc='center', handletextpad=0.3, handlelength=1, labelspacing=0.1,
    ...
    ax4 = plt.subplot(gs[4])  # 071829 #################################################################################
    nu = (3, 11)

    phiflow_file = f'charge_pump_FerHofSqu1_chi_50_t1_1_V_10_Coulomb_1_n_3_143_nphi_7_13_LxMUC_1_Ly_11_phi_0_11_111.dat'
    phiflow_path = os.path.join(phiflow_dir, phiflow_file)
    with open(phiflow_path, 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter='\t')
        phi = []
        ...
        for row in plots:
            ...
            charge.append(float(row[1]))
    charge = [i - charge[0] for i in charge]
    ax4.plot(phi, charge, '.', c=f'C1', marker=markers[2], fillstyle='none', markersize=5, markeredgewidth=0.2)
    ...
    fig.text(0.035, 0.28, "(e)", fontsize=12)
