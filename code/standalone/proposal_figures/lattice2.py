...
if __name__ == '__main__':
    ...
    for m in range(-6, 6):
        for n in range(-6, 6):
            ...
            ycoord = 0 + m*avec[0, 1]
            for i in range(3):
                ax.arrow(xcoord, ycoord, delta[i, 0], delta[i, 1], color='black', width=0.0001, ls='-', alpha=0.5)

    # for m in range(-6, 6):
    #     for n in range(-6, 6):
    #         xcoord = 0 + 2*n*avec[0, 0] + m*avec[0, 0]
    #         ycoord = 0 + m*avec[0, 1]
    #         ax.arrow(xcoord, ycoord, avec[0, 0], avec[0, 1], color='gray', width=0.0001, ls=':', alpha=0.5)
    #         ax.arrow(xcoord, ycoord, 2*avec[0, 0], 0, color='gray', width=0.0001, ls=':', alpha=0.5)
    #         ax.arrow(xcoord, ycoord, avec[0, 0], -avec[0, 1], color='gray', width=0.0001, ls=':', alpha=0.5)
    ...
    # ax.text(0.175 * avec[0, 0], 0.5 * avec[0, 1], 'a', fontsize=11)
    #
    # ax.annotate("", xy=(0, 0), xycoords='data', xytext=(avec[0, 0], 0), textcoords='data',
    #             arrowprops=dict(arrowstyle="<->", connectionstyle="arc3", color='k', lw=2))
    # ax.text(0.35 * avec[0, 0], -0.2 * avec[0, 1], 'b', fontsize=11)
    #
    # ax.annotate("", xy=(avec[0, 0], 0), xycoords='data', xytext=(avec[0, 0], (1 / 3) * avec[0, 1]), textcoords='data',
    #             arrowprops=dict(arrowstyle="<->", connectionstyle="arc3", color='k', lw=2))
    # ax.text(1.15 * avec[0, 0], 0.25 * (1 / 3) * avec[0, 1], 'c', fontsize=11)
    ...
    ax.set_xlim([-1.9*avec[0, 0], 1.9*avec[0, 0]])
    ax.set_xticks(np.arange(-1.9 * avec[0, 0], 1.9 * avec[0, 0]+0.1, step=2*avec[0, 0]))
    ax.set_ylim([-(1/3)*avec[0, 1], (5/3)*avec[0, 1]])
    ax.set_yticks(np.arange(-(1/3) * avec[0, 1], (5/3) * avec[0, 1]+0.1, step=avec[0, 1]))
    ...
    uc = Polygon(((0, 0), (avec[0, 0], (1/3)*avec[0, 1]), (avec[0, 0], avec[0, 1]), (0, (4/3)*avec[0, 1]), (-avec[0, 0], avec[0, 1]), (-avec[0, 0], (1/3)*avec[0, 1]), (0,0)), closed=True, fc=(1, 0, 0, 0.2))

    ax.arrow(0, 0, avec[0, 0], (1/3)*avec[0, 1], length_includes_head=True, head_width=0.05, facecolor='k')
    ax.arrow(avec[0, 0], (1 / 3) * avec[0, 1], 0, (2/3)*avec[0, 1], length_includes_head=True, head_width=0.05, facecolor='k')
    ax.arrow(avec[0, 0], avec[0, 1], -avec[0, 0], (1 / 3) * avec[0, 1], length_includes_head=True, head_width=0.05,
             facecolor='k')
    ax.arrow(0, (4/3)*avec[0, 1], -avec[0, 0], -(1 / 3) * avec[0, 1], length_includes_head=True, head_width=0.05,
             facecolor='k')
    ax.arrow(-avec[0, 0], avec[0, 1], 0, -(2 / 3) * avec[0, 1], length_includes_head=True, head_width=0.05,
             facecolor='k')
    ax.arrow(-avec[0, 0], (1/3)*avec[0, 1], avec[0, 0], -(1 / 3) * avec[0, 1], length_includes_head=True, head_width=0.05,
             facecolor='k')

    ax.text(-0.35, 0.45, "$\displaystyle \left| \sum_{\mathrm{plaquette}} \\theta_{ij} \\right|=2 \\pi n_{\\phi}$", fontsize=14)
...
########################################################################################################################
    ...
    plt.savefig("/home/bart/Documents/FSK2020/proposal/figures/lattice2.png", bbox_inches='tight', dpi=300)
    plt.show()