...
if __name__ == '__main__':
    ...
    ax.set_xlim([-4*avec[0, 0], 4*avec[0, 0]])
    ax.set_xticks(np.arange(-4 * avec[0, 0], 4 * avec[0, 0]+0.1, step=2*avec[0, 0]))
    ax.set_ylim([-2*avec[0, 1], 2*avec[0, 1]])
    ax.set_yticks(np.arange(-2 * avec[0, 1], 2 * avec[0, 1]+0.1, step=avec[0, 1]))
    ...
    uc = Polygon(((0, 0), (avec[0, 0], avec[0, 1]), (3 * avec[0, 0], avec[0, 1]), (2 * avec[0, 0], 0)), fc=(1, 1, 0, 0.2))
...
########################################################################################################################
    ...
    plt.savefig("/home/bart/Documents/FSK2020/proposal/figures/lattice1.png", bbox_inches='tight', dpi=300)
    plt.show()