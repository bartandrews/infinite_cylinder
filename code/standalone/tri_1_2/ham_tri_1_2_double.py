...
if __name__ == '__main__':
    ...
    eigenvalues2 = np.zeros((M2, numb_samples, numb_samples))
    ...
    for band in range(M2):
        for idx_x in range(numb_samples):
            ...
            for idx_y in range(numb_samples):
                ...
                idx = np.argsort(eigvals)
                eigenvalues2[band][idx_x][idx_y] = np.real(eigvals[idx[band]])
    ...
    for idx_x in range(numb_samples - 1):
        for idx_y in range(numb_samples - 1):
            berry_flux_matrix2[band1, idx_x, idx_y] = triple_berry_curv(eigenvectors2[:, band1, idx_x, idx_y],
                                                                        ...
                                                                        eigenvectors2[:, band3, idx_x, idx_y],
                                                                        eigenvectors2[:, band3, idx_x + 1, idx_y],
                                                                        eigenvectors2[:, band3, idx_x, idx_y + 1],
                                                                        eigenvectors2[:, band3, idx_x + 1, idx_y + 1])
            berry_flux_matrix2[band2, idx_x, idx_y] = berry_flux_matrix2[band1, idx_x, idx_y]
            berry_flux_matrix2[band3, idx_x, idx_y] = berry_flux_matrix2[band1, idx_x, idx_y]
    ...
    for i in range(M1):
        ...
        ax.plot_surface(KX, KY, E1[i], color=colors[i])
    ...
    E2 = np.zeros(M2, dtype=object)

    colors = plt.cm.RdBu_r(np.linspace(0, 1, M2))

    for i in range(M2):
        E2[i] = eigenvalues2[i, KX, KY]
        ax1.plot_surface(KX, KY, E2[i], color=colors[i])
    ...
    for i in [0, 3, 6]:
        ...
        ax1.text2D(-0.12, start + (i / 3) * interval,
                   "$C_{{{:2d},{:2d},{:2d}}}={:2d}$".format(i + 1, i + 2, i + 3, int(round(chern_numbers2[i]))),
                   color="k", fontsize=14)
    ...
    for i in range(M2 - 1):
        for kx in idx_x:
            for ky in idx_y:
                if abs(eigenvalues2[i, kx, ky] - eigenvalues2[i + 1, kx, ky]) < threshold:
    ...
    plt.savefig("/home/bart/Documents/papers/TBG/figures/tri_2_bands.png", bbox_inches='tight', dpi=300)
    plt.show()
