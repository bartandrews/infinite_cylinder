...
if __name__ == '__main__':
    ...
    for i in range(M1):
        ...
        if i == 0:
            ax.text2D(-0.105, start+i*interval, "$C_{{\pm{:2d}}}={:2d}$".format(i+1, 2*int(round(chern_numbers1[i]))),
                      color="k", fontsize=14)
        else:
    ...
    for i in range(M2):
        ...
        if i == 0:
            ax1.text2D(-0.105, start + i * interval,
                      "$C_{{\pm{:2d}}}={:2d}$".format(i + 1, 2 * int(round(chern_numbers2[i]))), color="k", fontsize=14)
        else:
