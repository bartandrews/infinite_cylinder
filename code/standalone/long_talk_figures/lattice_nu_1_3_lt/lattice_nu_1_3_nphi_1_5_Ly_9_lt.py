...
if __name__ == '__main__':
    ...
    ax.annotate("", xy=(3*avec[0, 0], -3*avec[0, 1]), xycoords='data', xytext=(12*avec[0, 0], 6*avec[0, 1]), textcoords='data',
               arrowprops=dict(arrowstyle="<->", connectionstyle="arc3", color='k', lw=2))
    ax.text(7.8 * avec[0, 0], 1.2 * avec[0, 1], '$L_y=9$', fontsize=14)

    ax.annotate("", xy=(2 * avec[0, 0], 6 * avec[0, 1]), xycoords='data', xytext=(12 * avec[0, 0], 6 * avec[0, 1]),
                textcoords='data',
                arrowprops=dict(arrowstyle="<->", connectionstyle="arc3", color='k', lw=2))
    ax.text(5.5 * avec[0, 0], 6.3 * avec[0, 1], '$L_x=1$', fontsize=14)
    ...
    ax.set_xlim([-7*avec[0, 0], 12*avec[0, 0]])
    ax.set_xticks(np.arange(-7 * avec[0, 0], 12 * avec[0, 0]+0.1, step=2*avec[0, 0]))
    ax.set_ylim([-3*avec[0, 1], 6*avec[0, 1]])
    ax.set_yticks(np.arange(-3 * avec[0, 1], 6 * avec[0, 1]+0.1, step=avec[0, 1]))
    ...
    muc = Polygon(((-7*avec[0, 0], -3*avec[0, 1]), (2*avec[0, 0], 6*avec[0, 1]), (12*avec[0, 0], 6*avec[0, 1]), (3*avec[0, 0], -3*avec[0, 1]+0)), fc=(1, 1, 0, 0.2))
    ...
    ax.scatter(-7 * avec[0, 0] + delta[1, 0] + 2 * 0 * avec[0, 0] + 6 * avec[0, 0],
               -3 * avec[0, 1] + delta[1, 1] + 6 * avec[0, 1], color='k', zorder=2, facecolors='k')

    #uc = Polygon(((0, 0), (avec[0, 0], avec[0, 1]), (3 * avec[0, 0], avec[0, 1]), (2 * avec[0, 0], 0)), fill=False, ec='g')

    fig.text(0.185, 0.9, "$n_\phi=1/5 \;\;\;\;\; \odot\mathbf{B}$", fontsize=14)
...
########################################################################################################################
    ...
    plt.savefig("/home/bart/Documents/presentations/2020_02_21/figures/lattice_nu_1_3_nphi_1_5_Ly_9_lt.png", bbox_inches='tight', dpi=300)
    plt.show()