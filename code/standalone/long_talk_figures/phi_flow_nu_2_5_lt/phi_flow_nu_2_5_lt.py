...
if __name__ == '__main__':
    ...
    ax1.plot(phi_FQH, FQH_charge[1], marker='>', linestyle='-', color='C1', label='FQH ($\kappa=0.1)$', markersize=5,
             zorder=2)
    ax1.plot(phi_FQH, FQH_charge[2], marker='<', linestyle='-', color='C2', label='$\mathbf{FQH}$ $(\kappa=0.2)$', markersize=5,
             zorder=3)

    for i, chi in enumerate(chi_range):
        if i == len(chi_range)-1:
            ax1.plot(phi_FQH, FQH_charge_kappa_limit[chi], marker='s', linestyle='-', color=s_m.to_rgba(chi), label='FQH ($\kappa=0.3$)', markersize=5, mec='k', zorder=5)
        else:
    ...
    fig.text(0.57, 0.86, 'IQH at $\kappa=1$', fontsize=11, verticalalignment='top', bbox=dict(boxstyle='round', facecolor='white', alpha=1))
    ...
    fig.text(0.57, 0.475, 'FQH at $\kappa=0.2$', fontsize=11, verticalalignment='top', bbox=dict(boxstyle='round', facecolor='white', alpha=1))
    ...
    fig.text(0.12, 0.025, "Hall response: $\sigma_\mathrm{H}=\\frac{e^2}{h}\\nu$", color="k", fontsize=14)

    # fig.text(0.36, 0.77, "$\\nu=1/3$", color="k", fontsize=14)
    # fig.text(0.15, 0.63, "$\\nu=1$", color="k", fontsize=14)

    fig.text(0.45, 0.63, "o", color="k", fontsize=25)
    fig.text(0.45, 0.36, "o", color="k", fontsize=25)

    fig.text(0.36, 0.77, "$C\\nu=2/5$", color="k", fontsize=12)
    fig.text(0.15, 0.63, "$C\\nu=1$", color="k", fontsize=12)

    fig.text(0.64, 0.92, "$\epsilon_\\alpha\equiv-\ln\Lambda_\\alpha^2$", color="k", fontsize=14)

    plt.savefig("/home/bart/Documents/presentations/2020_02_21/figures/phi_flow_nu_2_5_lt.png", bbox_inches='tight', dpi=300)
    plt.show()