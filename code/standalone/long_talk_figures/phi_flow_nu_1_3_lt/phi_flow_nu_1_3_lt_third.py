...
if __name__ == '__main__':
    ...
    ax1.plot(phi_FQH, FQH_charge[2], marker='<', linestyle='-', color='C2', label='FQH ($\kappa=0.2)$', markersize=5,
             zorder=3)
    ax1.plot(phi_FQH, FQH_charge[3], marker='^', linestyle='-', color='C4', label='$\\mathbf{FQH}$ ($\kappa=0.3)$', markersize=5,
             zorder=4)

    for i, chi in enumerate(chi_range):
        if i == len(chi_range)-1:
            ax1.plot(phi_FQH, FQH_charge_kappa_limit[chi], marker='s', linestyle='-', color=s_m.to_rgba(chi), label='FQH ($\kappa=0.4$)', markersize=5, mec='k', zorder=5)
        else:
    ...
    fig.text(0.64, 0.92, "$\epsilon_\\alpha\equiv-\ln"
                         "cd \Lambda_\\alpha^2$", color="k", fontsize=14)

    plt.savefig("/home/bart/Documents/presentations/2020_02_21/figures/phi_flow_nu_1_3_lt_third.png", bbox_inches='tight', dpi=300)
    plt.show()