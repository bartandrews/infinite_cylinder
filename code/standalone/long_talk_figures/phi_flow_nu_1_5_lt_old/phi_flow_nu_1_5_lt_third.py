...
if __name__ == '__main__':
    ...
    ax1 = plt.subplot(gs[:, 0], anchor=(0, 0.4))
    ...
    with open('charge_pump_FQH_chi.dat', 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter='\t')
        for row in plots:
    ...
    ax1.plot(phi_IQH, IQH_charge, marker='o', linestyle='-', c='C8', label='$\\mathbf{IQH}$ ($\chi=150$)', markersize=5, mec='k',
             zorder=1)

    ax1.plot(phi_FQH, FQH_charge[0], marker='d', linestyle='-', color='C5', label='FQH ($\chi=50$)', markersize=5,
             zorder=1)
    ax1.plot(phi_FQH, FQH_charge[1], marker='>', linestyle='-', color='C1', label='FQH ($\chi=100$)', markersize=5,
             zorder=2)
    ax1.plot(phi_FQH, FQH_charge[2], marker='<', linestyle='-', color='C2', label='$\\mathbf{FQH}$ ($\chi=150$)', markersize=5, mec='k',
             zorder=3)
    ...
    ax1.legend(bbox_to_anchor=(0.45, 1.35), loc="upper center", handletextpad=0.2, columnspacing=0.25, borderpad=0.4, framealpha=1, edgecolor='k',
               markerscale=1, fontsize=10, ncol=2)

    ax1.set_xticks(np.arange(0, 5.1, 1))
    ax1.set_xlim([0, 5])
    ax1.set_xlabel("$\Phi_x / 2\pi$", fontsize=11)
    ax1.set_yticks(np.arange(-5, 0.1, 1))
    ax1.set_ylim([-5, 0])
    ...
    ax1.axhline(-3, color='k', linewidth=0.5, ls='--')
    ax1.axhline(-4, color='k', linewidth=0.5, ls='--')
    ...
    left, bottom, width, height = [0.245, 0.25, 0.25, 0.25]  # left = 0.12
    ...
    ax2.axvline(3, color='k', linewidth=0.5, ls='--')
    ax2.axvline(4, color='k', linewidth=0.5, ls='--')
    ...
    ax2.arrow(4, 0, 0, 8, width=0.1, head_width=0, fill=False, zorder=3)
    ax2.arrow(5, 0, 0, 8, width=0.1, head_width=0, fill=False, zorder=3)
    ...
    ax3.arrow(5, 0, 0, 8, width=0.1, head_width=0, fill=False, zorder=3)
    ...
    fig.text(0.12, 0.05, "Hall response: $\sigma_\mathrm{H}\\propto C\\nu$", color="k", fontsize=14)

    fig.text(0.45, 0.49, "o", color="k", fontsize=25)
    fig.text(0.176, 0.49, "o", color="k", fontsize=25)

    fig.text(0.64, 0.92, "$\epsilon_\\alpha\equiv-\log\Lambda_\\alpha^2$", color="k", fontsize=14)

    plt.savefig("/home/bart/Documents/papers/long_talk/figures/phi_flow_nu_1_5_lt_third.png", bbox_inches='tight', dpi=300)
    plt.show()