...
if __name__ == '__main__':
    ...
    ax1 = plt.subplot(gs[:, 0], anchor=(0, 1))
    ...
    FQH_charge_kappa_05 = []
    with open('charge_pump_FQH_kappa_0.5.dat', 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter='\t')
        for row in plots:
            FQH_charge_kappa_05.append(float(row[1]))

    FQH_charge_kappa_06 = []
    with open('charge_pump_FQH_kappa_0.6.dat', 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter='\t')
        for row in plots:
            FQH_charge_kappa_06.append(float(row[1]))
    ...
    ax1.plot(phi_FQH, FQH_charge_kappa_04, marker='^', linestyle='-', color='C6', label='FQH $(\kappa=0.4)$',
             markersize=5,
             zorder=3)

    ax1.plot(phi_FQH, FQH_charge_kappa_05, marker='v', linestyle='-', color='C7', label='FQH $(\kappa=0.5)$',
             markersize=5,
             zorder=3)

    # ax1.plot(phi_FQH, FQH_charge_kappa_06, marker='x', linestyle='-', color='C8', label='FQH $(\kappa=0.6)$',
    #          markersize=5,
    #          zorder=3)
    ...
    ax1.legend(bbox_to_anchor=(0.45, -0.71), loc="lower center", handletextpad=0.2, columnspacing=0.25, borderpad=0.4, framealpha=1, edgecolor='k',
    ...
    left, bottom, width, height = [0.11, 0.215+0.15, 0.25, 0.25]  # left = 0.12
    ...
    # fig.text(0.035, 0.875, "(d)", color="k", fontsize=12)
    # fig.text(0.49, 0.875, "(e)", color="k", fontsize=12)
    # fig.text(0.49, 0.49, "(f)", color="k", fontsize=12)

    fig.text(0.115, 0.915, "Hall response: $\sigma_\mathrm{H}=\\frac{e^2}{h}C\\nu$", color="k", fontsize=14)

    fig.text(0.36, 0.76, "$C\\nu=2/5$", color="k", fontsize=12)
    fig.text(0.17, 0.64, "$C\\nu=1$", color="k", fontsize=12)

    fig.text(0.45, 0.66, "o", color="k", fontsize=25)
    fig.text(0.45, 0.385, "o", color="k", fontsize=25)

    fig.text(0.64, 0.91, "$\epsilon_\\alpha\equiv-\ln\Lambda_\\alpha^2$", color="k", fontsize=14)

    plt.savefig("/home/bart/Documents/presentations/2021_03_18/figures/phi_flow_nu_2_5_new.png", bbox_inches='tight', dpi=300)
    plt.show()