...
if __name__ == '__main__':
    ...
    # ax3.axhline(charge[0], color='k', linewidth=0.5, ls='--')
    # ax3.axhline(charge[-1], color='k', linewidth=0.5, ls='--')
    ...
    ax3.text(4.6, 0.33, "$\displaystyle{\sigma_\\text{H}=\\frac{3}{7}\\frac{e^2}{h}}$",
    ...
    # left, bottom, width, height = [0.215, 0.535, 0.25, 0.25]
    left, bottom, width, height = [0.11, 0.203, 0.25, 0.25]  # left = 0.12
    ax6 = fig.add_axes([left, bottom, width, height], projection='3d')
    ...
    for y in range(0, end, 2):
        ...
        z = r * np.cos(theta)
        x = r * np.sin(theta)
        ax6.scatter(x, y, z, label='parametric curve', c='b', s=point_size)

    # cut
    xx, zz = np.meshgrid(np.linspace(-7, 7, 10), np.linspace(-7, 7, 10))
    yy = (end - 1) / 2
    ax6.plot_surface(xx, yy, zz, alpha=0.25)

    # arrow
    ax6.quiver(0, end, 0, 0, -(end + end / 4), 0, arrow_length_ratio=0.06, color='k', lw=0.5)
    ax6.text2D(-0.078, 0.006, '$\Phi_x$', fontsize=11)
    ...
    z = r * np.cos(theta)
    x = r * np.sin(theta)
    ax6.plot_surface(x, y, z, alpha=0.25)
    ...
    z = r * np.cos(theta)
    x = r * np.sin(theta)
    ax6.plot(x, y, z, label='parametric curve', c='k', lw=0.5)

    theta = np.linspace(0, 2 * np.pi, 1)
    y = np.repeat(end, 1)
    z = r * np.cos(theta)
    x = r * np.sin(theta)
    ax6.scatter(x, y, z, label='parametric curve', c='k', marker='v', s=5)
    ax6.text2D(0.05, 0.03, '$L_y$', fontsize=11)

    ax6.view_init(elev=10., azim=-15)
    ax6.axis('off')
    ...
    for value in np.linspace(3, -3, 7, dtype=int):
        ...
        for i in range(len(x)):
            if z[i] == value:
                ...
                yvalue.append(y[i])
        if value != 0:
            ax4.scatter(xvalue, yvalue, marker='_', c='C{}'.format((value + 4) % 10), label='{}'.format(value))
        else:
            ax4.scatter(xvalue, yvalue, marker='x', c='C{}'.format((value + 4) % 10), label='{}'.format(value))
    ...
    ax4.set_xlabel("$k_a / \pi$", fontsize=11)
    ...
    plt.savefig("/home/bart/Documents/papers/TEE/case_study_3_7_kappa_2_3.png", bbox_inches='tight', dpi=300)
    plt.show()
