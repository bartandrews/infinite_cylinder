...
class FermionicHaldaneModel(CouplingMPOModel):
    ...
    def init_terms(self, model_params):
        ...
        for u1, u2, dx in [(0, 0, np.array([1, 0])), (0, 0, np.array([0, -1])),
                           (0, 0, np.array([-1, 1])),  # first triangle counter-clockwise
                           (1, 1, np.array([-1, 0])), (1, 1, np.array([0, 1])),
                           (1, 1, np.array([1, -1]))]:  # second triangle counter-clockwise
...
def run(phi_ext=np.linspace(0, 1.0, 9)):
    data = dict(phi_ext=phi_ext, QL=[], ent_spectrum=[])

    model_params = dict(conserve='N', filling=1/2.,
                        Lx=1, Ly=6,
                        t=-1., mu=0, V=0, phi_ext=0.,
...
def plot_model():
    ...
    M.coupling_terms['t2 Cd_i C_j'].plot_coupling_terms(ax, M.lat, text='t2 {strength_angle:.2f}')
    import pprint
    pprint.pprint(list(M.coupling_terms['t Cd_i C_j'].to_TermList()))
    ax.set_aspect(1.)
    plt.show()
    ...
    for phi_ext, spectrum in zip(data['phi_ext'], data['ent_spectrum']):
        for q, s in spectrum:
            ...
            ax.plot(phi_ext*np.ones(s.shape), s,
                    linestyle='', marker='_', color=color, label=label)
    ax.set_xlabel(r"$\phi / 2 \pi$")
