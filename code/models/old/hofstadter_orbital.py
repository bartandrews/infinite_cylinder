"""Hofstadter model with (x,y) orbitals.
...
class BosonicHofstadterOrbitalModel(CouplingMPOModel):
...
class FermionicHofstadterOrbitalModel(CouplingMPOModel):
    ...
    def init_terms(self, model_params):
        ...
        V = get_parameter(model_params, 'V', 100, self.name, True)
        phi_ext = 2 * np.pi * get_parameter(model_params, 'phi_ext', 0., self.name)
