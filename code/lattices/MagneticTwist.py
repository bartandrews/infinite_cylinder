"""Twist lattice in a perpendicular magnetic field (Landau gauge in x direction with numb_sites)."""
...
def plot_lattice():
    ...
    ax = plt.gca()
    fs = site.FermionSite()
    lat = MagneticTwist(5, 5, fs, basis=[[(numb_sites/4) * np.sqrt(3), numb_sites/4], [0, 1]])
    lat.plot_sites(ax)
