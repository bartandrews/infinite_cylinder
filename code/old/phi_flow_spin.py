...
def my_phi_flow_spin(model, lattice, initial_state, tile_unit, chi_max, J, Js, Jv, Lx, Ly, phi_min, phi_max, phi_samp):
    ...
    leaf = ("J_%s_Js_%s_Jv_%s_Lx_%s_Ly_%s_phi_%s_%s_%s.dat" % (J, Js, Jv, Lx, Ly, phi_min, phi_max, phi_samp))
    ...
    engine = f.define_iDMRG_spin_engine_pickle("phi_flow", model, lattice, initial_state, tile_unit, chi_max,
                                               J, Js, Jv, Lx, Ly, p.use_pickle, p.make_pickle, phi_min)

    for phi_ext in np.linspace(phi_min, phi_max, phi_samp):

        if phi_ext != phi_min:
            del engine.DMRG_params['chi_list']
            M = f.define_iDMRG_spin_model(model, lattice, J, Js, Jv, Lx, Ly, phi_ext)
