...
#
#for FILE_PATH in $(ls $SJOBS_FILES | sort -h -t '_' -k5,5n -k13,13n)  # sort by chi, then q
#do
...
do
	FILE="$(basename "$FILE_PATH")"  # strip the file from the path
	sbatch --output=$SLOGS/"$FILE".out --error=$SLOGS/"$FILE".err --job-name="$FILE" --mail-type=ALL \
	--mail-user=bandrews@physik.uzh.ch --qos=long --time=168:00:00 --mem=100G --ntasks=1 \
	--cpus-per-task=16 --threads-per-core=1 "$FILE_PATH"
done

SLOGS="/home/cluster/baandr/infinite_cylinder/slogs"
SJOBS_FILES="/home/cluster/baandr/infinite_cylinder/sjobs/Hof_FCIs/r_-4/*"
...
#
#SLOGS="/home/cluster/baandr/infinite_cylinder/slogs"
#SJOBS_FILES="/home/cluster/baandr/infinite_cylinder/sjobs/Hof_FCIs/r_4/*"
#
#for FILE_PATH in $(ls $SJOBS_FILES | sort -h -t '_' -k5,5n -k13,13n)  # sort by chi, then q
#do
#	FILE="$(basename "$FILE_PATH")"  # strip the file from the path
...
#
#SLOGS="/home/cluster/baandr/infinite_cylinder/slogs"
#SJOBS_FILES="/home/cluster/baandr/infinite_cylinder/sjobs/Hof_FCIs/r_-5/*"
#
#for FILE_PATH in $(ls $SJOBS_FILES | sort -h -t '_' -k5,5n -k13,13n)  # sort by chi, then q
#do
#	FILE="$(basename "$FILE_PATH")"  # strip the file from the path
